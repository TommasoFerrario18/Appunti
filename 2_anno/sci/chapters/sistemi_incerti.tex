...
I componenti base di questi sistemi sono:
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item  \textbf{sugeno}: regole if then con solo clausole formate da congiunzioni 
        e disgiunzioni di variabili linguistiche
    \end{itemize}
...
la situazione, questi devono poi essere mappati sugli insiemi fuzzy associati (fuzzyficaione),
successivamente avendo i fuzzy si  
il valore $x$ in u
Per esprimere le regole della conoscienza usiamo Mamdani, sintassi 
if and then con variabili linguistiche. tutto si basa sui fuzzy. La Fuzzy
partition è una famiglia di termini cherappresenta la variabile linguistica e 
l'universo è $[l,r]$. (insiemi fuzzy + regole sugli insiemi)

Le regole possono essere attivate insieme e le posso rappresentare come una tabella 
se le regole sono poche.

Nel processo decisionale abbimao.
\begin{itemize}
    \item fuzzycazione trasforma i valori  negli insiemi fuzzy (da numeri a gradi)
    \item applico le regole (inferenza), se le regole sono connesse da connettive 
    aggrego i fuzzy con tnorma o tconorma e ottengo un valore di uscita per il fuzzy
    in out per ogni regola (ogni regola ha un peso) da $m$ regole a $m$ valori
    del fuzzy di out e poi 
    devo aggregare per passare ad un valore. L'aggregazione si fa con max.
    \item defuzzificazione da fuzzy a valore numerico lo faccio usando la media o centro di gravità
\end{itemize}

media
Prendo l'intervalli di $x$ per cui ha $y$ massima e prendo la media nell'intervallo. 

centro di gravità
media pesata

Posso prima inferire poi aggregare o viceversa.


\begin{nota}
    Fuzzy set singleton è solo in un punto vale $1$ e in tutti gli altri $0$. In questo 
    cosa l'output lo posso esprime dal valore diretto.
\end{nota}

posso avere delle varianti dei sistmei mani, possiamo avere or e aggregare con 
tconorme ma coincide a definire regole singole.

Un altra variabile per esprimere le regole è takgino che esprime l'output con una 
funzione. Le regole mi danno già il valore numerico che devo aggregare con la media.
non devo fare la fuzzyficazione. Sistema compatto ma più difficile.

Per definire le regole serve un esperto, sistemi di questo tipo si trovano in forni,
frigorifero.

Oltre a questo c'è il fuzzy clustering.
