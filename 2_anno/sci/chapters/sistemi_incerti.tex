...
\begin{itemize}
    ...
    indefinito
    \item \textbf{irrilevante} (logica prist): non è associato al contesto
\end{itemize}
...
Alcune congiunzioni hanno un nome:
\begin{itemize}
...
\begin{definizione}
    ...
    \begin{itemize}
        ...
        \item \textbf{monotona a SX} $a\le b\implies a\ast c \le b\le c,\forall c$.
...
\section{Fuzzy set}
Abbiamo una funzione $f$ tale che $f(x)$ specifica il membership degree ovvero il grado 
di appartenenza.
...
Il fuzzy set è l'insieme delle coppie elemento e grado di appartenenza. Possono 
essere continui o discreti. Per gli insiemi continui ci sono alcune forme standard:
\begin{itemize}
    \item triangolari: dato $a<m<b$ allora sarà definito Così
    $$tr(x) = \begin{cases}
        0& x\le a\\
    ...
    \item trapezioidali: dati $a<m<n<b$ che nel caso $m=n$ mi riconduco al triangolare
    \item gaussiana: dove
...
\begin{definizione}
    Fs è normale se $\exists x, f(x) = 1$
\end{definizione}
\begin{definizione}
    Supporto è l'insieme tale che $\{x: f(x) >0\}$
\end{definizione}
\begin{definizione}
    Core è l'insieme tale che $\{x: f(x) =1\}$
\end{definizione}
\begin{definizione}
    $\alpha$-cut è l'insieme tale che $\alpha \in [0,1], \{x: f(x) \ge\alpha\}$
\end{definizione}
$alpha=1$ ottengo il core
\begin{definizione}
    strong $\alpha$-cut è l'insieme tale che $\alpha \in [0,1], \{x: f(x) >\alpha\}$
\end{definizione}
$alpha=0$ ottengo il support

I Fs sono degli insiemi e quindi possiamo definire $\cap, \cup, ^c$:
\begin{itemize}
    \item $(f\cap g)(x) = \min\{f(x), g(x)\}$ questo viene generalizzato dalle funzioni
    caratteristiche
