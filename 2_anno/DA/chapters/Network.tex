...
La matrice di \textbf{degree correlation} specifica la probabilità di trovare un 
un nodo di grado $i$ collegato ad un nodo di grado $j$ selezionando un arco a caso.
Questa matrice può essere calcolata come segue:
\begin{equation}
    e = \frac{E}{\|E\|}
\end{equation}
dove $E$ è la matrice che contiene il numero di archi che collegano un nodo di
grado $i$ con un nodo di grado $j$ e $\|E\|$ è il numero di archi della rete.

La probabilità $q_k$ che un nodo di grado $k$ sia alla fine di un arco selezionato
casualmente è data da:
\begin{equation}
    q_k = \frac{k \cdot p_k}{\langle k \rangle}
\end{equation}
dove $k$ è il grado, $p_k$ è la probabilità che un nodo abbia grado $k$ e 
$\langle k \rangle$ è il grado medio della rete.
