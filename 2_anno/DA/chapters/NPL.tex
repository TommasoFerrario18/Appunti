...
che compoaiono con frequenza maggiore. Di conseguenza è un processo non supervisionato.
...
\begin{nota}
      I topic li possiamo vedere come delle wordcloud e le etichette dei topic 
...
Si basa sulla teoria baiesiana quindi si dovrà campionare tutto quello che non 
è osservato.

Le assunzioni di questo modello sono:
\begin{itemize}
      ...
      \item ogni parola è formata uno dei topic
\end{itemize}

LDA ha due principi fondamentali:
\begin{itemize}
      \item modelli generativi del linguaggio perché permettono di generare parole di un 
      topic attraverso il campionamento di probabilità
      \item modelli inferenziali del linguaggio perché permettono di inferire un 
      topic date le osservazioni nel documento
\end{itemize}
...
\begin{itemize}
      \item \textbf{coerenza}: PMI, coerenza tra le parole e i topic
      \item \textbf{divergenza}: RBO, considera l'overlap delle liste di parole tra liste diverse e scala 
      \item \textbf{probabilistico}: KL, considera la divergenza tra la distribuzione 
      dei topi e la distribuzione target (metrica asimmetrica)
\end{itemize}
