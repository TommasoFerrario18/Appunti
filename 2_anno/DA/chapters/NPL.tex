...
I \textbf{Topic Models} sono modelli che permettono di identificare dei topical
pattern all'interno di un testo, sfruttando delle regolarità statistiche.
Una volta identificati i topic, si possono annotare i documenti sfruttando queste
etichette.

Ogni documento è un insieme di topic, mentre ogni topic una distribuzione di
probabilità su un insieme di parole.

Ogni topic può essere visto come una distribuzione di parole nel documento. Quindi,
possiamo avere una serie di documenti che contengono diverse parole, con frequenza
diversa. Vogliamo allora trovare i topic, i quali possono essere visti come una
rappresentazione latente dei documenti e parole.

I topic sono come delle wordcloud. Le etichette dei topic possono essere identificate
da un umano o automaticamente.

Una generica pipeline di topic modelling è riportata nella figura
\begin{figure}[!ht]
      \centering
      \includegraphics[width=0.5\textwidth]{./img/nlp/topic.png}
      \caption{Pipeline di Topic Modelling}
      \label{fig:topic}
\end{figure}
Alla base dei topic models ci sono le seguenti assunzioni:
\begin{itemize}
      \item Ogni documento è un insieme di topic: possiamo considerare che ogni
            documento contiene delle parole che sono associate a diversi topic.
      \item Ogni topic è un mix di parole: possiamo considerare ogni topic come
            un insieme di parole che descrivono lo stesso concetto.
\end{itemize}
Per la fase di riconoscimento e classificazione dei topic, si possono utilizzare
due differenti tipologie di modelli:
\begin{itemize}
      \item \textbf{Statistical-based}: tra questi abbiamo:
            \begin{itemize}
                  \item \textbf{Unigram model}
                  \item \textbf{Mixture of Unigram model}
                  \item \textbf{Latent Dirichlet Allocation}
            \end{itemize}
            Questi modelli possono essere valutati utilizzando diverse metriche
            come: PMI, RBO, KL.
      \item \textbf{Neural}: contextualized topic, i quali sono valutati con
            Word Embedding-based diversity. Il vantaggio di utilizzare questa
            tipologia di modelli è legato al fatto che permettono di apprendere
            relazioni tra le parole.
\end{itemize}
\subsection{Unigram model}
L'assunzione su cui si basa questo modello è che ogni documento è costituito da
un solo topic. Si usa il diagramma a piattine, il quale rappresenta le variabili,
ovvero le nostre parole. Le variabili possono essere osservate, colorate in grigio,
oppure non osservate, colorate in bianco.

Queste variabili sono contenute all'interno di un rettangolo, il quale rappresenta
il documento. In questo rettangolo è presente un pedice che rappresenta il numero
di parole che sono contenute nel documento. Infine, all'esterno del rettangolo
è presente un ulteriore rettangolo, il quale rappresenta la collezione di documenti,
anche esso decorato con un pedice che rappresenta il numero di documenti.

Questo modello è rappresentato con la seguente formula:
\begin{equation}
      p(\vec{w}) = \prod_{n = 1}^{N} p(w_n)
\end{equation}
$\vec{w}$ è un vettore di parole del dizionario, che possono essere osservate se
sono nei documenti o non osservate se non sono presenti documenti.
\subsection{Mixutere Unigram model}
Questo modello è una generalizzazione del modello precedente. Infatti, l'assunzione
su cui si basa è che in una collezione ci sono più topic, ma a ogni documento è
associato un solo topic.

In questo caso, ci si aspetta che nel topic implicano parole più probabili.
...
Dato un modello LDA, il quale consiste delle distribuzioni $P(w|z)$ e $P(z|d)$,
possiamo generare un documento campionando i topic e le parole nel seguente modo:
\begin{itemize}
      \item Campiono $P(z|d)$.
      \item Campiono $P(w|z)$.
\end{itemize}
