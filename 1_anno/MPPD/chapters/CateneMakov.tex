...
\begin{nota}
    ...
    transitiva.
\end{nota}
\begin{definizione}[\textbf{Irreducibilità}]
...
\begin{itemize}
    ...
    \item \textbf{Smoothing}: è un processo di calcolo della distribuzione di
          ...
          \begin{equation}
              \begin{array}{l}
                  b_{k+1:t} = P(e_{k+1:t}|X_k)=\sum_{x_{k+1}} P(e_{k+1:t}|X_k,x_{k+1})
                  P(x_{k+1}|X_k)=\sum_{x_{k+1}} P(e_{k+1:t}|x_{k+1}) P (x_{k+1}|X_k)= \\
          ...
          dove:
          \begin{itemize}
              \item $P(e_{k+1}|x_{k+1})$ è il modello sensoriale
              \item $P(e_{k+2:t} | x_{k+1})$ è la ricorsione
              \item $P(x_{k+1}|X_k)$ è il modello di transizione
          \end{itemize}
    ...
    \item \textbf{Spiegazione più probabile}: data una sequenza di osservazioni
          ...
          \end{equation}
          Supponiamo di essere in stati binari e che la sequenza delle osservazioni
