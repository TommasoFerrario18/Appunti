...
In questo modo non si controllano le probabilità di tutte le combinazioni di
valori ($2^n$) ma si controllano le probabilità di una successione finita di
...
la struttura della rete, alcuni esempi di strutture sono riportate in figura
...
variabili. Per calcolare l'inferenza esatta questo algoritmo permette, nel caso
di una rete Polytree, di computare la probabilità a posteriori per tutte le
variabili $\mathcal{O}(n)$, attraverso una query avente complessità $\mathcal{O}(n)$.
