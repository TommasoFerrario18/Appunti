...
\begin{esempio}
    ...
    \end{figure}
\end{esempio}
\subsection{Rappresentazione efficiente delle distribuzioni condizionali}
...
Per l'inferenza esatta si usano spesso \textbf{algoritmi di clustering} (join tree algorithms) per fondere dei 
...
Questo metodo mi permette di generare campioni \underline{senza considerare le
...
del tempo.  Le relazioni fra variabili casuali in istanti temporali diversi \dots
...
Un processo stocastico $\{X(t), t\in T\}$ è un insieme di variabilli aleatorie 
...
essere finiti o infiniti. Avremo tquindi diverse tipologie di processi stocastici:
\begin{itemize}
...
\begin{esempio}
    ...
    può rappresentare il numer odi visitatori al tempo $t$.
\end{esempio}

\begin{esempio}
    Un esempio di processo stocastico è il Random Walk, questo processo viene descrito 
...
I processi stocastici hanno una proprietà chiamata \textbf{proprietà Markoviana del 
...
$$P(X_{t+1} = i_{t+1 } | X_t =i_t, X_{t-1} = i_{t-2}, \dots, X_{t-0} = i_{t-0})=P(X_{t+1} = i_{t+1 } | X_t =i_t, X_{t-1} = i_{t-2}, \dots, X_{t-k} = i_{t-k}), k < \infty$$

Per la proprietà Markoviana del primo ordine formalmente sarà
$$P(X_{t+1} = i_{t+1 } | X_t =i_t, X_{t-1} = i_{t-2}, \dots, X_{t-0} = i_{t-0}) = P(X_{t+1} = i_{t+1 } | X_t =i_t)$$
...
$$P(X_{t+1} = i_{t+1 } ) = P(X_{t+1} = i_{t+1 } | X_t =i_t)$$
...
si conosce con certezza allora $\exists i, \pi_i = 0, \forall j \ne i, \pi_j=0$.
...
\begin{esempio}
    ...
    La probabilità iniziale $P(Sn) = \pi_{Sn}$.
\end{esempio}
