...
\begin{itemize}
    \item \textbf{Diagnostica}: si passa dagli effetti alle cause, l'evidenza è
          ...
          \begin{figure}[!ht]
              \centering
              \includegraphics[width=0.1\textwidth]{./img/Reti/Diagnostica.png}
    ...
    \item \textbf{Causale}: si passa dalle cause agli effetti, l'evidenza è
          ...
          \begin{figure}[!ht]
              \centering
              \includegraphics[width=0.1\textwidth]{./img/Reti/Causale.png}
    ...
    \item \textbf{Intercausale}: si passa l'informazione tra le cause di comuni
          ...
          \begin{figure}[!ht]
              \centering
              \includegraphics[width=0.25\textwidth]{./img/Reti/Intercausale.png}
    ...
    \item \textbf{mista}: combinazione di due o più delle tipologie di inferenza
          ...
          \begin{figure}[!ht]
              \centering
              \includegraphics[width=0.1\textwidth]{./img/Reti/Mista.png}
...
\begin{definizione} [\textbf{Sequenza di numeri casuale}]
    Una \textbf{sequenza di numeri casuale} è una sequenza di realizzazioni di
...
\end{nota}
Le proprietà statistiche dei numeri pseudocasuali sono le seguenti:
\begin{itemize}
...
\begin{esempio}
    ...
    \end{equation*}
\end{esempio}
\subsection{Moltiplicative Congruential Method}
...
\begin{equation}
    x_n = ax_{n-1} \mod M \quad 0 \leq x_n \leq M
\end{equation}
Con $a, M \in \mathbb{N}^+$.

Il problema di questo metodo è che genera sempre dei numeri in modo ciclico sulla
base del valore di $M$. Si può risolvere facilmente scegliendo $M$ molto grande
e primo.
\subsection{Linear Moltiplicative Congruential Method}
Una versione di quello precedente è la versione linear, ovvero aggiungere
un bias $c$, in modo tale da personalizzare la sequenza. In formule:
\begin{equation}
    x_n = (ax_{n-1} + c) \mod M
\end{equation}
...
\subsection{Generazione di una sequenza di numeri pseudocasuali derivanti da una
    distribuzione generica}
Vogliamo avere degli approcci per generare randomicamente dei numeri da una
distribuzione generica. Si utilizzano diversi metodi che a partire da un numero
random estratto in modo uniforme tra $0$ e $1$, generano numeri che rispettano
...
\begin{itemize}
    ...
    \item definita nel seguente modo $F(X=x) =P(X \leq x)$
\end{itemize}
...
Il metodo si basa sul generare coppie di numeri $\mu_i,\mu_j$ estratti da
distribuzioni $U[0-1]$, allora genereremo due valori.
\begin{equation}
    \begin{cases}
        x = a + (b-a)\mu_i \\
        y = cf_X(x)\mu_j
