...
Dove $V_join$ sono tutte le variabili, dove se hanno l'evidenza allora si vincola il 
valore e si utilizza il loro valore per campionare le singole variabili attraverso 
la distribuzione data dalle variabili date precedentemente rispetto all'ordine topologico.
Per ogni campione ottengo anche un peso $\omega_i$ che inizialmente è inizializzato 
a $1$, poi durante il campionamento si aggiorna $\omega_i = \omega_i\cdot P(E=e | Parents(E))$
dove $E$ è una delle variabili di evidenza e $P(E=e | Parents(E))$ si legge dalla 
CPT seguendo i valori delle variabili genitore precedentemente campionato.

Per una query la probabilità si calcola 
$$ P(G=V| N=V,B=V)=\frac{\sum_{s_i, G=V} \omega_i}{\sum_{s_i} \omega_i}$$
...
senza evidenza $X_i$, si sceglie casualmente la variabile da modificare, 
condizionalmente ai valori correntemente assunti dalle
variabili che costituiscono il Markov Blanket della variabile (nodo) $X_i$.
...
$$P(X_i | MarkovBlanket(X_i)) = \alpha \cdot P(X_i | parents(X_i)) \cdot \prod_{Y_i\in children(X_i)} P(Y_i| parents(Y_i))$$
