...
\begin{itemize}
    ...
    \item \textbf{Causale}: si passa dalle cause agli effetti, l'evidenza è
          ...
          \begin{figure}
              \centering
              \includegraphics[width=0.5\textwidth]{./img/Reti/Causale.png}
    ...
    \item \textbf{Intercausale}: si passa l'informazione tra le cause di comuni
          ...
          \begin{figure}
              \centering
              \includegraphics[width=0.5\textwidth]{./img/Reti/Intercausale.png}
...
approssimare queste variabili libere, campianandole generando dei numeri casuali.
...
\begin{definizione} [\textbf{Sequenza di numeri casuale}]
    Una \textbf{sequenza di numeri casuale} è una sequenza di realizzazioni di varibili aleatorie 
    indipendenti e identicamente distribuite
\end{definizione}

\begin{definizione} [\textbf{Sequenza di numeri pseudocasuali}]
    Una \textbf{sequenza di numeri pseudocasuali} è una sequenza di numeri che sembrano
    impredicibili da cui non si riesce ad estrarre alcuna regolarità
\end{definizione}

Quindi i numeri casuali non sono regolati da una particolare regola, al contrario 
i numeri pseudocasuali hanno delle regolarità e quindi ripetono un pattern.

\begin{nota}    
I metodi di generazione di numeri pseudocasuali implementati nel PC sono tutti 
basati sull'aritmetica 
\end{nota}

Le proprietà statistiche dei numeri pseudocasualicasuali sono:
\begin{itemize}
    \item \textbf{indipendenza statistica}: tutti i numeri della sequenza devono essere 
    indipendenti tra di loro
    \item \textbf{uniformità della distribuzione}: tutti i numeri della sequenza devono 
    essere estratti dalla stessa distribuzione
...
Un primo metodo di generare numeri casuali è attraverso l'utilizzo di una cifra 
di $10$ numeri, si eleva al quadrato e si prendono le $10$ cifre centrali. 
La sequenza non è casuale perché ogni numero è ottenibile da quello precedente,
inoltre si hanno problemi a causa dell'operazione di elevazione.
...
\paragraph{Moltiplicative Congruential Method}
Sia $x_0$ il seed della sequenza allora un generico numero pseudocasuale può essere 
generato da quello precedente nel seguente modo 
$$x_n=ax_{n-1}\mod M, \ 0\le x_n\le M$$
Con $a,M \in \mathbb{N}^+$.

Il problema di questo metodo è che genera sempre esattamente gli stessi numeri. 
Si può risolvere facilmente scelgiendo $M$ molto grande e primo.

\paragraph{Linear Moltiplicative Congruential Method}
Una versione di quello precedente è la verione linear, ovvero aggiungere 
un bias $c$, in modo tale da personalizzare la sequenza. 

$$x_n=(ax_{n-1} + c)\mod M, a,c>0, M>x_0,c,a$$.
In ogni caso abbiamo sempre la periodicità quindi $M$ si cerca di scegliere $M$ 
molto grande.

% TODO: chiedi al prof

\subsubsection{Generazione di una sequenza di numeri pseudocasuali derivanti da una
distribuzione generica}
Vogliamo avere degli approcci per generare randomicamente dei numeri da una distribuzione 
generica. Si utilizzano diversi metodi che a partire da un numero random estratto 
in modo uniforme tra $0$ e $1$, generano numeri che rispettano una particolare 
ditribuzione. Questi metodi sono:
\begin{itemize}
...
\paragraph{Trasformazione inversa}
Vogliamo generare una variabile aleatoria $X = x$ con una certa distribuzione. 
Si parte dalla sua \textbf{funzione di probabilità} ($f_X(x)$) scelta, si ricava la \textbf{funzione di 
probabilità cumulativa} ($F(X = x)$), quest'ultima funzione è:
\begin{itemize}
    \item continua
    \item monotona
    \item crescente
    \item limitata, compresa tra $[0,1]$
    \item definita nel seguente modo $F(X=x) =P(X\le x)$
\end{itemize}

La generazione si effettua in questo modo:
\begin{itemize}
    \item si seleziona la distribuzione che desideriamo 
    per la variabile $X$, definendo $f_X(x)$ e successivamente $F(X=x)$
...
Questo procedimento vale sia per Variabili discrete, sia per variabili continue.
Questo approccio coincide col campionamento della variabile $X$.

\begin{esempio}
    Sia $X$ la nostra variabile con $3$ simboli.
    $$P(X = x_0) = 0,1,P(X = x_1) = 0,3, P(X = x_2) = 0,6, F(X=x_i) = \sum_{l=0}^{i} P(X=x_l)$$

    Sia $F^{-1}$ la funzione inversa della funzione cumulativa. Allora 
    posso campionare tante volte la variabile $X$, generando tanti numeri casuali $\mu_i \in U([0-1])$.
...
Si può utilizzare per generare numeri $x$ derivanti da distribuzioni $X$ limitate da intervalli
finiti $[a,b]$. 

Scegliendo una distribuzione $f_X$ per $X$ tale che $X$ sia definita solo in $[a,b]$
e quindi $f_X : [a,b] \rightarrow [0,c]$.

Il metodo si basa sul generare coppie di numeri $\mu_i,\mu_j$ estratti da distribuzioni $U[0-1]$,
allora genereremo due valori 
...
se la coppia $(x_i, y_i)$ cade all'interno dell'area sottesa alla curva della funzione
$f_X(x)$ allora sarà accettata e verrà utilizzata per generare la sequenza di numeri 
casuali, altrimenti viene rigettata e si ripetono i passi fino a quando non troviamo 
una coppia ammessa.