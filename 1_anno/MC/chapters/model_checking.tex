...
dimostrare alcuni pezzi, ma dovremmo trovare un modo per unire i risultati.
...
\begin{enumerate}
    ...
    \item Si valuta se la formula è vera nel sistema di transizioni. Per
          effettuare questa operazione esistono degli algoritmi.
\end{enumerate}
...
\begin{definizione}[\textbf{Formule ben Formate per la logica LTL}]
    ...
    \begin{itemize}
        \item $\forall p \in AP$ si ha che $p \in FbF_{LTL}$, ovvero ogni
              proposizione atomica è una formula ben formata.
    ...
    \end{itemize}
\end{definizione}
Per definire la \textbf{semantica} utilizzeremo i \textbf{Modelli di Kripke} che prendono
...
Vediamo ora come associare una \textbf{semantica} alle formule ben formate. Per
fare ciò procediamo in due fasi:
\begin{enumerate}
    \item Definiamo un \textbf{criterio} per stabilire se una formula $\alpha$ è
          vera in un cammino massimale $\pi$.
    \item Diciamo che la formula è \textbf{vera} rispetto a uno stato $q$ se è
          vera in tutti i \textbf{cammini massimali} che partono da $q$.
\end{enumerate}
\begin{nota}
...
\end{nota}
Fissiamo quindi un cammino generico $\pi$ e consideriamo una formula ben formata
$\alpha$ allora:
\begin{equation}
              ...
              \begin{itemize}
                  \item \textbf{Operatori logici}: supponiamo $\alpha, \beta \in FBF_{LTL}$
                        \begin{itemize}
                            ...
                            \item $\pi \vDash \alpha \lor \beta \iff (\pi \vDash
                                      \alpha) \lor (\pi \vDash \beta) $
                        \end{itemize}
...
\begin{esempio} [Esercizio d'esame]
    ...
    \begin{itemize}
        ...
        \item $AG (AF \ q)$: non vale in $s_2$ perché possiamo rimanere
              all'infinito, anche negli altri stati essendo che $s_2$ è sempre raggiungibile sarà falsa.
    \end{itemize}
\end{esempio}
\begin{nota}
...
\begin{itemize}
    ...
    \item Trasformare le espressioni ottenute sostituendo i nomi degli stati con
          le proposizioni atomiche che sono vere in un determinato stato.
        ...
        \item Costruire i cammini massimali dei due modelli.
              \begin{itemize}
                  ...
                  \item Modello $M_2$:
                        ...
                        \end{equation}
              \end{itemize}
        \item Sostituire i nomi degli stati con le proposizioni atomiche che sono
...
\begin{definizione}[\textbf{Relazione d'ordine parziale}]
    Data una relazione binaria su un'insieme $A$, ovvero $\leq \subseteq A \times
        A$, allora $\leq$ è una \textbf{relazione d'ordine parziale} se è:
    \begin{itemize}
...
\begin{definizione}[\textbf{Funzione monotona}]
    Siano due insiemi parzialmente ordinati $(A,\leq)$ e $(B,\leq)$. Diremo che
    una funzione $f: A \to B$ si dice \textbf{monotona} se, per ogni $x, y
        \in A$, allora:
    \begin{equation}
...
una formula $LTL$, quello che si fa è analizzare stringhe infinite di proposizioni
...
\begin{itemize}
    ...
    \item Etichetto gli archi dell'automa di Büchi con l'insieme delle proposizioni
...
\end{nota}
\section{$\mu$ calcolo}
Il \textbf{$\mu$ calcolo} è un linguaggio logico che permette di definire delle formule
ricorsive.
\begin{esempio}
