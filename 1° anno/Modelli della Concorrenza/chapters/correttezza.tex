...
\begin{equation}
    \begin{array}{ccccccc}
        \toprule
        \text{A} & \text{B} & A \land B & A \lor B & \lnot A & A \implies B & A \iff B \\
        \midrule
        F        & F        & F         & F        & T       & T            & T        \\
        F        & T        & F         & T        & T       & T            & F        \\
        T        & F        & F         & T        & F       & F            & F        \\
...
\begin{definizione}[\textbf{Dimostrazione}]
    Una dimostrazione è definita come una catena di regole $A_1,\dots, A_n \vdash
        B$, ovvero da $A_1, \dots, A_n$ si deriva $B$.
\end{definizione}
...
\textit{precondizioni} e \textit{postcondizioni} che sono delle formule della
...
dove:
\begin{itemize}
    \item $\{\alpha\}$ è una formula che rappresenta le pre-condizioni.
...
\begin{itemize}
    \item Espressioni aritmetiche $E$ definite come:
          \begin{equation}
              \forall z \in \mathbb{Z} \ z \in E \ \land \ \forall e_1, e_2 \in
              E \ (e_1 + e_2), (e_1 - e_2), -e_1, (e_1 \ast e_2), (e_1 / e_2),
              (e_1 \% e_2) \in E
          \end{equation}
    \item Espressioni logiche $B$ definite come:
          ...
          \end{equation}
          \begin{equation}
              (b_1 \& b_2), (b_1 | b_2), !b_1 \in B \ \land \ \forall e_1,e_2\in
              E (e_1 < e_2), (e_1 == e_2),(e_1 != e_2 ), (e_1 <= e_2)\in B
          \end{equation}
    \item Comandi $C$ definiti come:
          \begin{itemize}
              ...
              \item Operatore di iterazione $\text{while} \ B \ \text{do} \ C \
                        \text{endwhile}$ dove $B \in C$ è un espressione logica
                    e $C$ è un comando.
          \end{itemize}
\end{itemize}
...
\begin{enumerate}
    \item Per il comando \textbf{skip} definiamo la seguente regola di derivazione:
          ...
          sono le stesse.
    \item Per il comando \textbf{sequenza} definiamo la seguente regola di
          derivazione:
          \begin{equation}
    ...
    \item Per il comando \textbf{assegnamento} definiamo la seguente regola di
          derivazione:
          \begin{equation}
...
condizione $q$ e chiamiamo le istruzioni in sequenza $V$, $W$ e $Z$. Inoltre,
supponiamo che $V$ e $Z$ non contengano istruzioni di iterazioni.
\begin{equation}
    \{p\} \ V; \ W; \ Z \ \{q\}
\end{equation}
