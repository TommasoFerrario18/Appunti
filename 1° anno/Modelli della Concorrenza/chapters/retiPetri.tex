...
L'insieme $C_\Sigma$ è un insieme \textbf{finito} perché si parla di sottoinsiemi di insiemi 
finiti che sono anch'essi finiti, ovviamente esplode in un esponenziale.
...
\begin{itemize}
    \item \textbf{Comportamento sequenziale}: sequenze di occorrenze o di eventi
        ...
        \end{equation}
    \item \textbf{Comportamento non sequenziale} (semantica a passi): sequenze di passi (”step semantics”)
        ...
        \end{equation}
    \item \textbf{Comportamento non sequenziale}  (semantica di ordine parziale): processi non sequenziali
...
\begin{definizione}
    ...
    il sistema di transizioni etichettato $SCG_\Sigma=(C_\Sigma, E, A,c_{in})$ dove:
    \begin{equation}
...
\begin{definizione}[\textbf{Processi non sequenziali}]
    ...
    $\langle N = (B, E, F), \phi \rangle$ è un processo non sequenziale su $\Sigma$
    se e solo se:
    \begin{itemize}
...
\begin{definizione}[\textbf{Rete di occorrenze}]
    ...
    \begin{itemize}
        \item \textbf{conflitti solo in avanti}: $\forall b \in B: \ | ^{\bullet}b| \leq 1$ sono presenti dei conflitti
              solo in avanti.
        \item \textbf{aciclica}: $\forall x, y \in B \cup E: \ (x, y) \in F^{+} \Rightarrow (y, x) \notin F^{+}$
              non ci sono cicli
        \item\textbf{passato finito}: $\forall e \in E: \{x \in B \cup E| \ xF^{\ast} e \}$ è finito
...
Se i due elementi non sono ordinati allora saranno o in relazione $co$ o in 
\textbf{conflitto}. Estenderemo la relazione $co$ specificando $x\text{ co }y$ 
aggiunge il fatto che $\lnot x\#y$.
...
\begin{definizione}
    ...
    sistema può essere rappresentato da un'unica matrice $\underline{N}:S\times T\rightarrow\mathbb{N}$ 
...
\begin{equation}
    M_0[\sigma>M_1\implies \underline{M_0}+\underline{N}\cdot \underline{c_\sigma}=\underline{M_1}
\end{equation}
...
\begin{itemize}
	...
	\begin{itemize}
        ...
        di stato} che descrivono la dinamica e le $S$-invarianti e le $T$-invarianti
