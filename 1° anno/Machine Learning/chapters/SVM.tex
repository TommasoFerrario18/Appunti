...
\end{enumerate}
Il margine funzionale specifica quanta è buona la classificazione ma non non misura
...
scala. Bisogna studiare la distanza $d$ del punto $x$ dall'iperpiano. Si ha quindi:
\begin{equation}
...
rispento all'iperpiano, in questo modo il margine rimane invariato. Al contrario
...
\begin{equation*}
    \begin{aligned}
        \max \frac{1}{\|w\|} \\ \text{s.t.} \ y_i(\langle w, x_i \rangle + b) \leq \gamma
    \end{aligned}
\end{equation*}
...
\begin{definizione}[\textbf{Kernel trick}]
    ...
    prodotto interno dele trasformate dei due vettori $x$ e $y$, tramite $\Phi$,
