...
Per prima cosa diremo data la stringa $Q\in \Sigma^\ast$ 
\begin{definizione} [$Q$-intervallo rispetto alla BWT]
...
\end{definizione}
\begin{definizione} [$Q$-intervallo rispetto al SA]
...
\end{definizione}
Si può osservare che $\varepsilon$-intervallo coicide con l'intervallo $[1,n+1)$.
\begin{esempio}
...
Dato un $Q$-intervallo si possono effettuare le seguenti osservazioni:
\begin{itemize}
    \item Se noi leggiamo tutti gli indici del SA di un $[b,e)\equiv Q$-intervallo,
    questi rappresentano gli indici dei suffissi che hanno come prefisso $Q$. Quindi
    rappresentano le occorrenze di $Q$ nel testo.
    \item Se noi leggiamo gli indici del BWT di un $[b,e)\equiv Q$-intervallo, 
    sono i simboli precedenti ai suffissi che hanno come prefisso $Q$. Quindi 
    rappresentano i simboli precedenti all'occorrenza di $Q$ nel testo.
\end{itemize}
...
\begin{esempio}
    Dall'esempio precedente il $aca$-intervallo corrisponde a $[4,6)$ quindi saranno un
    totale di $6-4= 2$ occorrenze di $aca$ nel testo, iniziano alle posizioni 
    $1,5$ del testo e sono precedute dai simbli $\$,a$
\end{esempio}

Successivamente definiamo
\begin{definizione}
    la backward extension di un $Q$-intervalo $[b,e)$ con il carattere $\sigma$ è il
    $\sigma Q$-intervalo, cioè l'intervallo relativo alla stringa ottenuta concatenando 
    il simbolo $\sigma$ con $Q$.
\end{definizione}
\begin{esempio}
...
L'algoritmo di ricerca esatta per BWT è composto dai seguenti passi:
\begin{enumerate}
    ...
    \begin{itemize}
        \item se il risultato è l'intervallo vuoto allora $P$ non ha occorrenze in $T$
        quindi si termina l'esecuzione 
        \item se il risultato non è l'intervallo vuoto allora:
        \begin{itemize}
            \item se $i>1$ allora si calcola $i--$ e si ripete il punto $2$
            \item se $i=1$ allora ho trovato il $P$-intervallo e si ritorna l'occorrenza
        \end{itemize}
    \end{itemize}
\end{enumerate} 
...
\begin{itemize}
    \item Per capire se $P$ occorre in $T$ allora si hanno al massimo $m$ iterazioni sul 
    testo in cui calcoliamo il $Q'$-intervallo
    \item Se occorre $P$ in $T$ allora servono un totale di $m$ iterazioni, 
    altrimenti saranno minori
    \item Se $P$ non occorre in $T$,
    supponendo che $p$ è la posizione di $P$ per cui il $P[p,m]$-intervallo è vuoto,
    allora l'ultimo suffisso che occorre in $T$ è $P[p+1,m]$ in un numero di iterazioni
    $P[p+1,m]+1$
    \item La complessità quindi sarà $O(mlogn)$
\end{itemize}

Possiamo rimuovere il $logn$ nell'equazione del tempo rendendo costante 
il calcolo della backward extension. Possiamo osservare che per un $Q$-intervallo,
e dato un simbolo $\sigma$:
\begin{itemize}
...
\end{itemize}
Da notare che, la backward extension viene definita dalle posizioni
limite prese dal SA alla posizione BWT coincidente al carattere da cercare.
...
Quindi ora si può mostrare l'algoritmo di ricerca esatta
...
Algoritmo è lineare ($O(m)$) solo se definiamo il calcolo 
costante di $LF$.
...
La struttura composta da due funzioni:
\begin{itemize}
    \item $c: \Sigma \to \left\{0,1,2,3,\dots,n\right\}$: dove 
    \begin{equation}
        c(\sigma) = numero di simboli della BWT che sono < \sigma    
    \end{equation}
    \item $occ:  \left\{0,1,2,3,\dots,n+1\right\}\times \Sigma\to \left\{0,1,2,3,\dots,n\right\}$: dove 
    \begin{equation}
        Occ(i,\sigma) = numero di simboli di \sigma che occorrono in B[1,i-1]   
    \end{equation}
\end{itemize} 
\begin{esempio}
    $T=ggtcagtc\$$
    ...
    $$c(t) = 7$$
\end{esempio} 
$c(\sigma)$ ci dice il numero di suffissi che iniziano con un simbolo $<\sigma$ e 
la massima posizione nel SA di un suffisso che inizia con un simbolo $<\sigma$.
\begin{esempio}
    $BWT=cctt\$aggg$
    $$Occ(1,\sigma) = 00000$$
    $$Occ(2,\sigma) = 00100$$
...
Definiamo il calcolo di LF in tempo costante
\begin{osservazione}
    prop.
\end{osservazione}

$LF(i)$ la possiamo ottenere calcolando il numero di suffissi che iniziano con un
carattere più piccolo di $B[i]$ con un il numero di simboli si $B[i]$ che occorrono
nella BWT prima della posizione $i$. 
$$LF(i) = c(B[i]) + Occ(i, B[i]) +1$$