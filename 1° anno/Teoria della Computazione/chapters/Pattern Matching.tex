...
Le strutture di indicizzazione sono strutture che manipolano l'input, occupano
molta memoria, ma permettono di rendere più efficiente l'interrogazione sull'input.
La loro costruzione è molto complessa.
...
\end{definizione}
Il suffisso del carattere determinatore è il carattere terminatore stesso.
\begin{definizione}
...
Sono stati inventati da Myers e Manber nel 1990, non contiene l'informazione sui
simboli del testo, occupa $\Theta(nlogn)$ e la ricerca esatta di un pattern $P$ 
su un testo $T$ in $|P|log|T|$.

\begin{definizione}
    Il suffix array di un testo $T$ lungo $n$ è un array $S$ lungo $n$, tale che
    $S[i]= q$ se e solo se il $q$-suffisso è l'$i$-esimo suffisso nell'ordinamento
    lessicografico dei suffissi di $T$. 
\end{definizione}

\begin{esempio}
    Supponiamo che $T=ggtcagtc\$$ allora costruiamo il suffix array.
    
    Per prima cosa elenchiamo tutti i suffissi che iniziano ad ogni posizione del testo $T$:
    $$s[1] = ggtcagtc\$$$
    $$s[2] = gtcagtc\$$$
    $$s[3] = tcagtc\$$$
    $$s[4] = cagtc\$$$
    ...
    $$s[8] = c\$$$
    $$s[9] = \$$$
    Ora riordiniamo i suffissi per ordine lessicografico:
    $$s[9] = \$$$
    $$s[5] = agtc\$$$
    $$s[8] = c\$$$
    $$s[4] = cagtc\$$$
    ...
    $$s[7] = tc\$$$
    $$s[3] = tcagtc\$$$
    Quindi nel suffix array avvrò le posizioni di inizio dei suffissi ordinati:
    \begin{equation}
        SA = \left[ 9,5,8,4,1,6,2,7,3 \right]
    \end{equation}
\end{esempio}

Per effettuare la ricerca esatta del pattern sul suffix array ci serve la struttura
già costruita e il testo e verrà effettuata in un $O(|T|log|T|)$.

Per prima cosa si può osservare che: % aggiungere esempi alle osservazioni
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \label{passo-2-sa} Si considera il suffisso di indice $S[p]$ con $p=\left\lfloor\frac{R-L}{2}\right\rfloor$,
    in tempo $O(m)$ si controlla:
    \begin{itemize}
...
\begin{itemize}
    ...
    \item $r[n]$ è uguale a $T[q-1]$ se $q>1$
    \item $r[n]$ è uguale a $T[n]$ se $q=1$ 
    
\end{itemize}
Il punto 2 implica che:
\begin{itemize}
    \item L'ultimo simbolo della $q$-rotazione è il simbolo iniziale della 
    $q-1$-rotazione 
    \item il $q-1$-suffisso contiene il $q$-suffisso come suffisso se $q>1$
\end{itemize}
Il punto 3 implica che:
\begin{itemize}
    \item l'ultimo simbolo della $q$-rotazione è il simbolo iniziale della $n$-rotazione
    (e quindi dell'$n$-suffisso)
\end{itemize}
...
\begin{esempio}
    ...
    ... LA B è l'ultima colonna 
\end{esempio}
La costruzione della $BWT$ si effettua in tempo % dp non sono riuscito a copiare

% riguarda le proprietà
Considerando la prima colonna della $BWT$ di un testo allora si ha la certezza che $F$,
il vettore della prima colonna, è sempre ordinato, in aggiunta si può osservare
che per ogni posizione $i$, il simbolo $B[i]$ precende nel testo il simbolo $F[i]$.
