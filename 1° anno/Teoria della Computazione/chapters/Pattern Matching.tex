...
Definiremo 
\begin{definizione} [(rispetto a BWT)]
...
\end{definizione}
\begin{definizione} [(rispetto a SA)]
    $Q$-intervallo è l'intervallo $[b,E)$ di posizioni SA che contengono
    i simboli che precedono i suffissi che condividono la striga $Q$ come prefisso 
\end{definizione}
\begin{esempio}
    % trascrivi esempio    
\end{esempio}
Se noi leggiamo tutti gli indici del SA di un $[b,e)$ $Q$-intervallo rappresentano gli
indici dei suffissi che hanno come prefisso $Q$. (occorrenze di $Q$ nel testo)

Se noi leggiamo gli indici del BWT sono i simboli precedenti ai suffissi che hanno
come prefisso $Q$. (simboli precedenti all'occorrenza di $Q$ nel testo).

Il numero di occorrenze di $Q$ in $T$ sarà $e-b$ se $[b,e)$.

\begin{esempio}
    Sia $aca$-intervallo allora questo corrisponde a $[4,6)$ quindi saranno un
    totale di $6-4$ occorrenze di $aca$ nel BWT.
\end{esempio}

\begin{definizione}
    la backward extension di $\sigma$ con un $Q$-intervalo allora significa calcolare
    il $\sigma Q$-intervallo
\end{definizione}

L'algoritmo di ricerca consiste nel:
\begin{enumerate}
    \item suffisso $\epsilon$ del pattern $P$
...
Per capire se $P$ occorre in $T$ allora si hanno al massimo $m$ iterazioni sul 
testo in cui calcoliamo il $Q'$-intervallo. Se occorre $P$ in $T$ allora servono
un totale di $m$ iterazioni, altrimenti saranno minori. La complessità quindi sarà
$O(mlogn)$. Per rendere il calcolo di costante la backward extension dobbiamo:
\begin{itemize}
    ...
    \item ...
\end{itemize}
Da notare che backward extension il nuovo intervallo viene definito dalle posizioni
limite prese dal SA alla posizione BWT coincidente al carattere da cercare.

Algoritmo che non è lineare perché non abbiamo definito il calcolo costante di LF.
