...
che non lo rende un o-piccolo di $n$. Memorizzo quindi ogni $l-$esimo valore
$rank(i)$, a questo punto, a query time scorriamo i restanti $l-1$ bit.
...
third $T$, di dimensioni $2^{k - 1} \times k - 1$. $T$ salva i valori di $rank(i')$
per tutte le posizioni $k - 1$ in tutte le possibili configurazioni indotte dai
blocchi definiti per $S$. $T$ richiede uno spazio pari a $\mathcal{O}(\sqrt{n}
    \log n \log \log n)$ bit.
...
e accedere alla lookup table dove la riga è indicizzata da $c_i$ e la colonna
dalla posizione relativa in tempo $\mathcal{O}(1)$.
\begin{equation}
    rank(B) = \begin{cases}
...
\begin{esempio}
    ...
    da effettuare accesso in tempo costante. Scegliamo i valori di $l = 9$ e
    $k = 3$ e otteniamo:
    \begin{figure}[!ht]
        \centering
...
Analizziamo ora una rappresentazione basata sulla visita in left-to-rigth
level-order di un albero, ovvero una visita in ampiezza. In questa rappresentazione
si considera un nodo detto \textbf{super-root}. A questo punto si memorizza la
sequenza $D$ dei gradi dei nodi. Questa sequenza viene memorizzata utilizzando un
prefix code binario, dove per ogni nodo visitato si aggiunge a un bitvector $B$
i simboli della sequenza $1^d0$, con $d$ che rappresenta il grado del nodo. La
super-root viene aggiunta in modo che il numero di 1 presenti nella sequenza sia
uguale al numero di nodi dell'albero.
...
$T$ un insieme di $| \Sigma | = s$ stringhe binarie $B_\sigma[1\dots n], \
    \forall \sigma \in \Sigma$ si ha che:
\begin{equation}
...
in due. A ogni nodo $v$ corrisponde una sequenza chiamata $R_v$ (sotto-sequenza
dell'input $T$), la quale è sotto-sequenza di quella con cui è etichettato il
nodo genitore di $v$. Alla root corrisponde la sequenza $R_v = T$.
...
\begin{itemize}
    \item \textbf{rank}: si inizia il calcolo nel nodo root e si determina a
          ...
          Nella radice se $B_v [j] = 0$ allora $R_v [j] = \sum \left[\left\lceil
                  \frac{s}{2} \right\rceil\right] \lor R_v [j] \prec \Sigma
              \left[\left\lceil \frac{s}{2} \right\rceil\right]$, altrimenti se
          $B_v[j] = 1$ allora il carattere che si cerca è nella seconda metà
          dell'alfabeto.
          ...
          \begin{itemize}
              \item se $\sigma = \Sigma\left[\left\lceil \frac{s}{2} \right\rceil
                            ...
                            \frac{s}{2} \right\rceil\right]$
                    allora si prosegue verso il "figlio" di sinistra, aggiornando
                    $i$ con $i = rank_{0,B_v}(i)$.
          ...
          se si è andati a sinistra e $\Sigma = \Sigma\left[\left\lceil
                  \frac{s}{2} \right\rceil + 1 \dots s\right]$ se a destra.
...
\begin{enumerate}
    \item Si etichetta la root con $R_v = T$ e un bitvector $B_v$ tale che
          ...
          \begin{itemize}
              \item $B_v[i] = 0 \iff T[i] = \Sigma\left[\left\lceil \frac{s}{2}
    ...
    \item Posso quindi cancellare $T$ e costruire i bitvector $B_{v1}$ e $B_{v2}$,
          ...
          fino al raggiungimento delle foglie (quando si raggiungono alfabeti di
          cardinalità 1).
\end{enumerate}
...
Su strutture dati semplici come array matrici$\dots$ posso eseguire operazioni
...
quindi i tempi di accesso richiedonoun tempo paria a $\theta(n)$ ma nella realtà
è difficile che accada quindi accesso in $\theta(1 + \alpha)$ nel caso migliore,
con il numero di posizioni nella hash table proporzionale al numero di elementi
della tabella (quindi $\alpha \to 1$), ho accesso in tempo $\theta(1)$.
...
sui falsi positivi, ovvero ottengo $T$ e non $F$ con probabilità $\mathcal{P}
    \leq \delta$
...
per $\mathcal{U}$ con $h_j : \mathcal{U} \to \{0, \dots, m - 1\}$, con $m =
    \frac{n}{\delta}$. Prendendo casualmente una funzione di hash $h \in
...
inoltre, si considera una famiglia di $l$ funzioni hash ideali $\mathcal{H} =
    \{h_0, \dots, h_{l - 1}\}$ dove:
\begin{equation}
